...
Dalam tutorial ini, kami akan memandu kamu melalui pembuatan 
aplikasi pemungutan suara dasar.

Kami akan berasumsi bahwa kamu telah [menginstal Blitz](./get-started). 
Kamu dapat mengetahui apakah Blitz telah berhasil terinstal, dan versi mana yang kamu miliki 
dengan menjalankan perintah berikut di terminal-mu:

```sh
blitz -v
```

Jika Blitz terinstal, kamu akan melihat versi penginstalan-mu. 
Jika tidak, kamu akan mendapatkan pesan kesalahan yang mengatakan 
seperti ini “command not found: blitz”.

## Membuat aplikasi baru {#membuat-aplikasi-baru}
