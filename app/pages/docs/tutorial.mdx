...
Dalam tutorial ini, kita akan membuat aplikasi voting dasar.

Kami akan berasumsi bahwa kamu sudah [menginstal Blitz](./get-started).
Untuk mengetahui apakah kamu telah menginstalnya, jalankan perintah
berikut di terminal:
...
Jika Blitz terinstal, kamu akan melihat versi instalasi-nya. Dan jika kamu
tidak memilikinya, di terminal akan mendapatkan kesalahan seperti "command
not found: blitz".

## Membuat aplikasi baru {#creating-a-new-app}

Dari terminal, `cd` ke direktori tempat kamu ingin membuat aplikasi dan
...
Blitz akan membuat direktori bernama `my-blitz-app` di direktori kamu saat
ini. Kamu akan diminta untuk memilih form library. Untuk tutorial ini,
pilih library yang direkomendasikan: `React Final Form`.

Mari kita lihat apa yang `blitz new` buat:
...
File-file ini adalah:

- Folder `app/` adalah container untuk sebagian besar proyek kamu. Di
  sinilah kamu akan meletakkan halaman dan route API kamu.

- Folder `app/pages/` adalah folder utama untuk menyimpan halaman. Jika
  kamu telah menggunakan Next.js, kamu akan segera melihat bahwa itu
  berbeda. Di Blitz, kamu dapat memiliki beberapa folder halaman yang akan
  digabungkan saat kompilasi.

- Folder `app/core/` adalah tempat utama untuk meletakkan components,
  hooks, dll. yang digunakan di seluruh aplikasi kamu.

- `db/` adalah tempat konfigurasi database kamu. Jika kamu menulis model
  atau menangani migrasi, ini adalah tempat untuk meletakkannya.

- Di `public/` semua file statis kamu berada. Jika kamu memiliki gambar,
  foto, video, atau file lain, mereka masuk ke dalam folder `public/`.

- `.babelrc.js`, `.env`, dll. ("dotfiles") adalah file konfigurasi untuk
  berbagai alat yang digunakan dalam aplikasi.

- `blitz.config.js` adalah untuk menyesuaikan Blitz (lanjutan). Ini
  memiliki format yang sama dengan next.config.js.

- `tsconfig.json` adalah pengaturan yang kami rekomendasikan untuk
  TypeScript.

## Server pengembangan {#the-development-server}

Sekarang, pastikan kamu berada di dalam direktori `my-blitz-app` dan
jalankan perintah berikut:
...
Kamu akan melihat output berikut di baris perintah:
...
Itu berarti server pengembangan sedang berjalan. Pergi ke
[localhost:3000](http://localhost:3000) di browser kamu. Jika kamu melihat
halaman splash dengan logo Blitz, Artinya kita berhasil!

## Daftar sebagai pengguna {#sign-up-as-a-user}

Aplikasi Blitz dibuat dengan registrasi dan login yang telah dibuat
sebelumnya. Mari kita coba. Klik tombol **Sign Up** Masukkan alamat email
dan kata sandi apa saja dan klik **Create Account**. Kemudian, kamu akan
diarahkan kembali ke awal, di mana kamu akan melihat `id` dan `role`
pengguna kamu.

Jika mau, kamu dapat mencoba menutup sesi, memulainya lagi, dan bahkan
mengklik **Forgot your password?** dan lihat cara kerjanya.

## Tulis halaman pertama-mu {#write-your-first-page}

Selanjutnya mari kita buat halaman pertama kamu.

Buka file `app/pages/index.tsx` dan ganti konten component `Home` dengan
yang berikut ini:
...
Simpan file dan kamu akan melihat pembaruan halaman di browser-mu. Kamu
bisa menyesuaikan ini sebanyak yang diinginkan. Saat kamu siap, lanjutkan
